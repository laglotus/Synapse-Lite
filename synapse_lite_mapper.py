...
def _resolve_autoswitch_profile(target: str) -> str:
    """Resolve autoswitch target to a concrete profile.
...
def autoswitch_tick(rgb_worker=None) -> None:
    ...
    target = (mapping_lc.get(cls) if isinstance(mapping_lc, dict) else None)
    if target is None:
        target = fallback
    target = str(target or fallback)
    target = _resolve_autoswitch_profile(target)
...
def _apply_openrgb_device(
    ...
    # Retry once with a longer timeout (OpenRGB can be briefly busy / device can be waking)
    try:
        ok_r, msg_r = _run_openrgb(args, timeout=8)
        if ok_r:
            return True, msg_r
    except Exception:
        pass
